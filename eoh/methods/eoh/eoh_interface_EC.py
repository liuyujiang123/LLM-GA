...
class InterfaceEC:
    def __init__(self, pop_size, m, api_endpoint, api_key, llm_model, llm_use_local, llm_local_url, debug_mode,
                 interface_prob, select, n_p, timeout, use_numba, **kwargs):
    ...
    def population_generation(self, worst):
        ...
        for i in range(n_create):
            _, pop = self.get_algorithm([], 'i1', worst)
            for p in pop:
                population.append(p)
    ...
    def get_offspring(self, pop, operator, device_id, worst):
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(self.interface_eval.evaluate, code, device_id)
                fitness = future.result(timeout=self.timeout)
                if fitness is None or fitness > worst:
                    if operator == "e1":
                        self.bad_ideas_e1.append(offspring['algorithm'])
                    elif operator == "e2":
                        self.bad_ideas_e2.append(offspring['algorithm'])
                    elif operator == "m1":
                        self.bad_ideas_m1.append(offspring['algorithm'])
                    elif operator == "m2":
                        self.bad_ideas_m2.append(offspring['algorithm'])
    ...
    def get_algorithm(self, pop, operator, worst):
        results = []
        gv.total_val += self.pop_size
        try:
            results = Parallel(n_jobs=self.n_p, timeout=self.timeout + 15)(
                delayed(self.get_offspring)(pop, operator, i, worst) for i in range(self.pop_size))
        except Exception as e:
            if self.debug:
                print(f"Error: {e}")
