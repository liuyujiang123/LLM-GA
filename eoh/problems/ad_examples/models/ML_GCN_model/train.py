...
def main_open_images():
    global args, best_prec1, use_gpu
    args = parser.parse_args()

    use_gpu = torch.cuda.is_available()

    train_dataset = Open_Images('/home/lyj/fiftyone/open-images-v7/validation', 'train')
    val_dataset = Open_Images('/home/lyj/fiftyone/open-images-v7/validation', 'val')

    num_classes = 601

    # load model
    model = gcn_resnet101(num_classes=num_classes, t=0.4, adj_file='/home/lyj/fiftyone/open-images-v7/validation/metadata/open_images_adj.pkl')
    ...
    state = {'batch_size': args.batch_size, 'image_size': args.image_size, 'max_epochs': args.epochs,
             'evaluate': args.evaluate, 'resume': args.resume, 'num_classes': num_classes, 'difficult_examples': True,
             'save_model_path': '/home/lyj/LLMs_Multi_Labels/eoh/problems/ad_examples/checkpoint/mlgcn/open_images',
             'workers': args.workers, 'epoch_step': args.epoch_step, 'lr': args.lr}
    if args.evaluate:
        state['evaluate'] = True
    engine = GCNMultiLabelMAPEngine(state)
    engine.learning(model, criterion, train_dataset, val_dataset, optimizer)
...
def generate_adj_voc2007():
    ...
    a = torch.zeros(20)
    b = torch.zeros((20, 20))
    dataset = Voc2007Classification('../../data/VOC2007', 'train')
    ...
    dataset = Voc2007Classification('../../data/VOC2007', 'val')
    dataset.transform = data_transforms
    loader = torch.utils.data.DataLoader(dataset, batch_size=128,
                                         shuffle=False,
                                         num_workers=2)
...
def generate_adj_voc2012():
    ...
    dataset = Voc2012Classification('../../data/VOC2012', 'train')
    dataset.transform = data_transforms
    loader = torch.utils.data.DataLoader(dataset, batch_size=128,
                                         shuffle=False,
                                         num_workers=2)
...
def generate_adj_open():
    ...
    a = torch.zeros(num_class)
    b = torch.zeros((num_class, num_class))
    dataset = Open_Images('/home/lyj/fiftyone/open-images-v7/validation', 'train')
    dataset.transform = data_transforms
    loader = torch.utils.data.DataLoader(dataset, batch_size=128,
                                         shuffle=False,
                                         num_workers=2)
    ...
    for idx, (input, target) in enumerate(loader):
        ...
        # 更新 a: 累加每个类别的总数
        a += torch.sum(target, dim=0)

        # 更新 b: 使用外积来快速计算类别间的共现次数
        # 注意这里要排除掉 i == j 的情况，因为不需要统计同一个类自身的共现
        outer_product = torch.matmul(target.T, target)
        mask = 1 - torch.eye(num_class, device=target.device)  # 创建掩码以去除对角线元素
        b += outer_product * mask

        # 如果需要确保 b[i][j] 和 b[j][i] 相同，可以做如下处理
        b = (b + b.T - torch.diag(torch.diag(b))) / 2
    ...
    adj['nums'] = a.numpy()
    adj['adj'] = b.numpy()
    f = open('/home/lyj/fiftyone/open-images-v7/validation/metadata/open_images_adj.pkl', 'wb')
    pickle.dump(adj, f)
    print(a)
    print(b)

def generate_word2vec_open():
    import gensim
    df = pd.read_csv('/home/lyj/fiftyone/open-images-v7/validation/metadata/classes.csv', header=None)
    tags = list(df[1])
    word2vec = np.zeros((601, 300))
    glove_file = '/home/lyj/LLMs_Multi_Labels/eoh/problems/ad_examples/models/ML_GCN_model/GloVe/glove.840B.300d.txt'
    model = gensim.models.KeyedVectors.load_word2vec_format(glove_file, binary=False)
    for i, c in enumerate(tags):
        word2vec[i] = model[c]

    f = open('/home/lyj/fiftyone/open-images-v7/validation/metadata/glove_open_images.pkl', 'wb')
    pickle.dump(word2vec, f)


if __name__ == '__main__':
    # main_open_images()
    generate_word2vec_open()
    # generate_adj_open()
    # main_voc2007()
    # main_voc2012()
    # main_nuswide()
